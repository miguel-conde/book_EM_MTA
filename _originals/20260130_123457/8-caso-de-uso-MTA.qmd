...
* checks: conservación de masa $\sum_k \hat z = C$ y evaluación vs ground truth

Cuando lo ejecutes, si quieres, lo siguiente es extender el M-step a **$\lambda=\exp(X\beta)$** (regularización, pooling por tipo de campaña/crea), igual que en el HCP–Brick.
...
> **la mayor parte de la señal explicativa está en esta transformación**, no en (X\beta).
...
y usar $\gamma$ para **modular la saturación** (capítulo avanzado).
...
La confusión es totalmente normal porque **$\omega_{b,h}$** (o $w_{h,b}$) **no es una sola cosa**, sino un **concepto** que puede instanciarse de varias formas coherentes.

Voy paso a paso y al final te dejo una **regla mental** para no perderte.

---

# Qué es realmente $\omega_{b,h}$
...
> que tiene la unidad (h) para generar contribución dentro del agregado (b)

No es “probabilidad”, no es “share” por sí sola, y **no tiene por qué sumar 1**.

---

# Las tres interpretaciones válidas de $\omega_{b,h}$

## 1️⃣ Exposición binaria: $\{0,1\}$ (la más simple)
...
$$
z_{h,b} \sim \operatorname{Poisson}(\lambda_h)
$$

📌 Aquí **toda la diferenciación** viene de $\lambda_h$.
...
## 2️⃣ Exposición como **intensidad** $[0, +\infty)$ (la más habitual)
...
> a igual productividad $\lambda_h$, más exposición → más contribución esperada

### Importante

* $\omega$ **no tiene por qué estar normalizada**
* no tiene por qué sumar 1 por brick
* puede ser grande (1000 imps) o pequeña (0.2 GRPs)

---

## 3️⃣ Peso normalizado $[0,1]$ (caso especial, NO obligatorio)
...
* Sí afecta a la escala de $\lambda_h$ (pero no a los shares)

📌 Por eso verás implementaciones con $[0,1]$ y otras con valores brutos.

---

# Entonces… ¿qué es $\omega_{b,h}$ *en tu cabeza*?

La forma correcta de pensarlo es:

> $\omega_{b,h}$ = **“cuánto puede contribuir $h$ al brick $b$, antes de saber si es bueno o malo”**

Luego:

* $\lambda_h$ = *qué tan bueno es $h$*
* $\omega_{b,h}$ = *cuánta oportunidad tiene*
...
❌ “$\omega$ es el share dentro del brick”
...
> $\omega$ **ya incorpora** carryover y saturación.

---

# Regla mental (la más importante)

> **Si quitas $\lambda_h$, $\omega_{b,h}$ debe seguir teniendo sentido como “oportunidad de generar contribución”**
...
* y parece que $\omega$ “tiene que ser” una cosa concreta

No:
👉 **$\omega$ es una elección de modelado**, y lo importante es que sea:
...
* o ver qué pasa si eliges una $\omega$ “mala” (ejemplos de fallo del EM)
...
| Exposición                  | $\omega_{b,h}$                                    | $\omega_{m,k,t,g}$                                            |
| Exposición cruda            | pertenencia / visitas                             | impresiones / GRPs                                            |
| Exposición efectiva         | visitas ponderadas                                | Hill(Adstock(imps))                                           |
| Modelo latente              | $z \sim \operatorname{Poisson}(\lambda\omega)$           | $z \sim \operatorname{Poisson}(\lambda\omega)$ (o $\operatorname{Gamma}$) |
...
| $\omega$  | Oportunidad / intensidad de exposición  |
...
> Solo cambia qué significa $\omega$ y qué significa el total observado.**
