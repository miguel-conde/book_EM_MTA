...
Cuando entrenes EM con **solo** ($y_b$), ($w_{h,b}$) y ($X_h$), podrás comparar:
...
```         
hcp_df   →  define λ_h = exp(X_h β)    ↓ hb_df    →  define dónde y cuánto se expone cada HCP (w_{h,b})    ↓ y_df     →  observa solo la suma por brick 
```
