...
-   $M_{h,b} \in \{0,1\}$: pertenencia HCP–brick\
    (o peso si la pertenencia no es binaria)

### 👻 Latente (no observado)

-   $z_{h,b}$: contribución de ventas del HCP $h$ al brick $b$

con la restricción: $$
\sum_{h \in b} z_{h,b} = y_b
$$

📌 Este $z_{h,b}$ es **la variable oculta clave**.
...
$$
p(\text{venta} \mid b) = \sum_{h \in b} \pi_{h,b} \cdot p_h
$$

-   HCPs = componentes

-   $\pi_{h,b}$ = peso del HCP en el brick

-   $z_{h,b}$ = responsabilidades
...
-   Unidades → $\theta_h = \lambda_h$ (Poisson)
...
-   $w_{h,b}$: exposición / peso del HCP en el brick\
...
perfecto asi. tengo 2 preguntas: 1) En Mixture Models explicas $z_{h,b}$ = responsabilidades pero no aprece en la formula (aparece $p_h$) 2) En pocas lineas, dime sobre credit attribution y relacion con MTA en marketing - solo para entender, no entraremos mas en este curso

------------------------------------------------------------------------

Muy buenas preguntas — ambas van **al núcleo conceptual** del modelo. Las respondo con precisión y sin desviar el curso.

## 1️⃣ Mixture Models: ¿dónde están exactamente las *responsabilidades* ($z_{h,b}$)?
...
p(y) = \sum_{k=1}^K \pi_k \cdot p(y \mid \theta_k)
$$
...
\prod_{h \in b}
\left[
\pi_{h,b}^{z_{h,b}} \cdot p(z_{h,b} \mid \theta_h)
\right]
$$
...
-   $p_h$ describe **cómo genera ventas un HCP**

-   $\gamma_{h,b}$ dice **qué fracción del brick le pertenece**
...
-   $p_h$ = distribución del componente

-   $z_{h,b}$ = variable latente

-   $\gamma_{h,b} = \mathbb{E}[z_{h,b}]$ = responsabilidades
