...
Tomamos (w\_{h,b}=1) si el HCP pertenece al brick, 0 si no.

## 2️⃣ Inicialización (iteración 0)

Inicializamos productividades (tasas) arbitrarias, todas positivas:

# \[ \\lambda\^{(0)} = (\\lambda_1,\\lambda_2,\\lambda_3,\\lambda_4)
...
Usamos:\
\[\
\\lambda_h\^{(1)}=\\frac{\\sum_b \\hat z\_{h,b}}{\\sum_b w\_{h,b}}\
\]
...
-   (p\_{2,A} = 26.67/86.67 \\approx 0.3077)

-   (p\_{3,A} = 26.67/86.67 \\approx 0.3077)
...
-   (p\_{3,B}\\approx 0.3636)

-   (p\_{4,B}=20/73.34\\approx 0.2727)
...
-   H1: 0.4224 → (\\hat z\_{1,A}=42.24)

-   H2: 0.2888 → (\\hat z\_{2,A}=28.88)

-   H3: 0.2888 → (\\hat z\_{3,A}=28.88)

**Brick B:** (26.30, 26.30, 16.36), suma = 68.96\
Shares:

-   H2: 0.3814 → (\\hat z\_{2,B}=22.88)

-   H3: 0.3814 → (\\hat z\_{3,B}=22.88)

-   H4: 0.2372 → (\\hat z\_{4,B}=14.23)
...
No está “prediciendo” (y_b) (eso ya está dado). Está encontrando (\\lambda_h) tal que:
...
-   **M-step** recalcula (\\lambda) como “ventas esperadas / exposición”
