...
Pasamos de “un (\\lambda_h) libre por HCP” a un modelo **paramétrico, generalizable y regularizado**.
...
\[\
z\_{h,b} \\sim \\text{Poisson}(\\exp(X_h\\beta), w\_{h,b})\
\]
...
# \[ \\boxed{ \\mathbb{E}\[z\_{h,b}\\mid y_b,\\beta\^{(t)}\]

y_b\
\\cdot\
\\frac{\\exp(X_h\\beta\^{(t)}) w\_{h,b}}\
{\\sum\_{h'\\in b}\\exp(X\_{h'}\\beta\^{(t)}) w\_{h',b}}\
}\
\]
...
\[\
\\tilde y_h = \\sum_b \\mathbb{E}\[z\_{h,b}\]\
\]

y la exposición total:

\[\
\\tilde w_h = \\sum_b w\_{h,b}\
\]

Entonces el M-step es:

## \[ \\boxed{ \\max\_\\beta \\sum_h \\left\[ \\tilde y_h \\log \\lambda_h
...
-   E-step: reparto proporcional a (\\theta_h w\_{h,b})
...
-   Aparece en muchas ecuaciones:\
    \[\
    y_b \\approx \\sum_h \\exp(X_h\\beta) w\_{h,b}\
    \]
